describe('Edit Specific Supervisor Test', () => {
  it('should login and edit a specific supervisor successfully', () => {
    ...
    cy.visit('https://list.sanjoydeyreju.com/supervisor');

    // Step 3: Find the row that contains the supervisor name and click Edit within that row
    cy.contains('td', 'Test Supervisor')  // Find supervisor's name cell
    ...
    cy.wait(500);
    cy.get('input[name="name"]').clear().type('Test Supervisor2');
    ...
    cy.get('input[type="checkbox"][value="১০ সাতারকুল"]').check({ force: true });
